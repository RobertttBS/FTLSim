...
class FTL:
	...
	def erase_block(self, block, backup_block = False):
		# print("Erase", block.id) # Changed to print() if uncommented
	...
	def process(self, RW, lba, sector_count):
		# Get target LPNs
		page_start = lba // NAND.SECTORS_PER_PAGE # Integer division
		page_end = (lba + sector_count + NAND.SECTORS_PER_PAGE - 1) \
					// NAND.SECTORS_PER_PAGE # Integer division
	...
	def select_victim_block(self):
		...
		for b in self.blocks:
			...
			# float division is default in Py3, so float(b.valid_pages) is not strictly needed
			# but kept for clarity if b.valid_pages could be 0, though guarded above.
